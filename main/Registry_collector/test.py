...
def main():
    if len(sys.argv) < 2: # argv는 커맨드 라인에서 전달될 인자를 담고 있다. 최소 0, 1 은 있어야 한다는 말.
                            # cmd 에서 python [스크립트 이름] [경로] 이렇게 인자가 0, 1 은 있어야 실행되고 안되면
        print("사용법: python [스크립트이름].py [경로(=NTFS Volume)]") # 사용법 메시지 출력
        sys.exit(0)
    # 접근하려는 경로 설정
    volume = "\\\\.\\C:" #c드라이브
    ext = Extract_file(volume)
    ext.extract('/$MFT', '$MFT') # 첫 번째 인자 : 특정 시스템 파일 / 두 번째 인자 : 로컬 시스템에 저장할 때 사용할 파일명
    ext.extract('/$LogFile', '$LogFile') # 같음.
    ext.extract_usnjrnl()
