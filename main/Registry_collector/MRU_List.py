...
def print_collected_data(hive, subkey):
    ...
    if not collected_data:
        print(f"No data collected from {subkey}")
        pass
...
def save_results_to_file(result):
    artifact_path = os.path.join('./ARTIFACT/REGISTRY/', 'MRU_List.txt')
...
def run():
# MRU 문서와 실행명령 내용을 프린트함.
    hive = winreg.HKEY_CURRENT_USER
    print_collected_data(hive, r"Software\Microsoft\Windows\CurrentVersion\Explorer\RecentDocs")
    #print_collected_data(hive, r"Software\Microsoft\Windows\CurrentVersion\Explorer\RunMRU")
